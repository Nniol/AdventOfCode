...
def get_coord(coord_as_str:str)->Coord:
    ...
    coord:Coord= Coord(x = int(coord[0]),y=int(coord[1]))
    return coord
...
for line in lines:
    ...
    for coord in coords:
        a_coord = get_coord(coord)
        if a_coord.y > greatest_y:
            greatest_y = a_coord.y
   

lines = get_lines_from_file(day_number=14)

CAVERN_WIDTH=10000
CAVERN_HEIGHT= greatest_y+3 # Code calls for plus 2, but need to be 1 more to account for zero starting array
CAVERN = np.full((CAVERN_WIDTH,CAVERN_HEIGHT),'.')
...
for x in range(CAVERN_WIDTH):
    CAVERN[x][CAVERN_HEIGHT-1]='#'

view_start = Coord(450,0)
view_end = Coord(550,CAVERN_HEIGHT-1)
...
try:
    while True:
        ...
        try:
            ...
            sand_grain_count+=1
            if is_same_point(grain,SAND_START):
                raise IndexError()
